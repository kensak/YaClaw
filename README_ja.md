...
- **ACP対応** — ACP対応のCLIエージェント（Copilot、Codex、Gemini、OpenCode等）とチャンネルが stdio JSON-RPC 2.0 で接続。豊富な機能がSNS上で利用可能です。
- **セレクトメニュー** — モデルやモード、思考レベルを選択するメニューを表示
- **許可ボタン** — ツール使用時の許可をボタンで選択
- **画像や音声ファイルに対応**  — エージェントの応答に含まれるファイルはDiscordにアップロード
