...
class Channel(ABC):
  ...
  async def __start_queue(self):
    await log("trace", f"Channel {self.channel_name}: Starting queue...")
    while True:
      response = await self.response_message_queue.get()
      await log("trace", f"Channel {self.channel_name}: Got message from queue: {response}")
      await self.handle_response_message(response)

  @final
  async def __start(self):
    await log("trace", f"Channel {self.channel_name}: Starting queue and listener...")
  ...
  async def create_request_skeleton(self):
    request = {"from": self.channel_name, "to": self.channel_settings["agent"], "body": ""}
  ...
  async def handle_request_message(self, request):
    ...
    await log("trace", f"Channel {self.channel_name}: Received message: {request_message}")
    await log("trace", f"Channel {self.channel_name}: Putting message in queue for agent {request_message['to']}...")

    to_ = request_message['to']
    if type(to_) == list:
      to_ = to_[0]
    agent = AgentManager.get_agent(to_)
    if agent is None:
      await log("error", f"Channel {self.channel_name}: No agent found with name '{to_}'. Skipping...")
      return

    await agent.request_message_queue.put(request_message)
    await log("trace", f"Channel {self.channel_name}: Message put in queue for agent {request_message['to']}.")
...
class ChannelManager:
  ...
  async def initialize(cls, channel_settings):
    ...
    for channel_name, settings in channel_settings.items():
      await log("trace", f"ChannelManager: Initializing channel {channel_name}...")

      plugin_name = settings.get("plugin", None)
      if plugin_name is None:
        await log("error", f"ChannelManager: No plugin specified for channel {channel_name}. Aborting...")
        return False
      ...
      if plugin_name not in sys.modules:
        ...
        await log("trace", f"ChannelManager: {plugin_name} plugin already loaded. Reusing...")
      ...
      for attr in dir(module):
        ...
        if isinstance(class_, type) and issubclass(class_, Channel) and class_ != Channel:
          break
      if class_ is None:
        await log("error", f"ChannelManager.initialize: No Channel derived class found in plugin {plugin_name} for channel {channel_name}. Aborting...")
        return False
      ...
      if not await channel_instance._Channel__initialize(channel_name, settings):
        await log("error", f"ChannelManager.initialize: Failed to initialize channel {channel_name} with plugin {plugin_name}. Aborting...")
        return False
