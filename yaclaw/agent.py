...
class Agent(ABC):
  ...
  @final
  async def __start_queue(self):
    await log("trace", f"Agent {self.agent_name}: Starting queue...")
    while True:
      request = await self.request_message_queue.get()
      await log("trace", f"Agent {self.agent_name} got message from queue: {request}")
  ...
  async def __start(self):
    ...
    async with asyncio.TaskGroup() as tg:
      ...
      task2 = tg.create_task(self.__start_queue())
  ...
  async def __handle_request_message(self, request):
    await log("trace", f"Agent {self.agent_name} is handling request message : {request}")
    await self.handle_request_message(request)
    await log("trace", f"Finished handling request message for agent {self.agent_name}.")

  @final
  async def create_response_skeleton(self, request):
    response = {"via": [], "body": ""}
    if not "reply_to" in request:
      response["reply_to"] = request["from"]
    else:
    ...
    if to_[0] != self.agent_name:
      await log("warning", f"Handler for agent {self.agent_name}: Message is not destined for this agent. Message: {request}")
    response["from"] = to_[0]
    response["to"] = to_[1:]
    if response["to"] == []:
      response["to"] = response["reply_to"]
      del response["reply_to"]
    elif len(response["to"]) == 1:
      response["to"] = response["to"][0]
  ...
  async def handle_response_message(self, response):
    ...
    if "reply_to" in response:
      dest_agent_name = response["to"] if type(response["to"]) == str else response["to"][0]
      dest_agent = AgentManager.get_agent(dest_agent_name)
      await log("trace", f"Agent {self.agent_name}: Putting response message in queue for agent {dest_agent.agent_name}...")
      await dest_agent.request_message_queue.put(response)
      await log("trace", f"Agent {self.agent_name}: Response message put in queue for agent {dest_agent.agent_name}.")
    else:
      channel = ChannelManager.get_channel(response["to"])
      await log("trace", f"Agent {self.agent_name}: Putting response message in queue for channel {channel.channel_name}...")
      await channel.response_message_queue.put(response)
      await log("trace", f"Agent {self.agent_name}: Response message put in queue for channel {channel.channel_name}.")
  ...
  async def initialize(cls, agent_settings):
    ...
    for agent_name, settings in agent_settings.items():
      ...
      if plugin_name is None:
        await log("error", f"AgentManager: No plugin specified for agent {agent_name}. Aborting...")
        return False
      ...
      if plugin_name not in sys.modules:
        ...
        await log("trace", f"AgentManager: {plugin_name} plugin already loaded. Reusing...")
        module = sys.modules[plugin_name]

      # register Agent derived class
      for attr in dir(module):
        class_ = getattr(module, attr)
        if isinstance(class_, type) and issubclass(class_, Agent) and class_ != Agent:
          break

      if class_ is None:
        await log("error", f"AgentManager.initialize: No Agent derived class found in plugin {plugin_name} for agent {agent_name}. Aborting...")
        return False
      ...
      if not await agent_instance._Agent__initialize(agent_name, settings):
        await log("error", f"AgentManager.initialize: Failed to initialize agent {agent_name} with plugin {plugin_name}. Aborting...")
        return False
