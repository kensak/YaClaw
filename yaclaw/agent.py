...
class Agent(ABC):
    ...
    async def log(self, type_, message):
        await yaclaw.log.log(self.agent_name, type_, message)
    ...
    async def __start_handler(self):
        await self.log("trace", "Starting handler...")
    ...
    async def __start_queue_handler(self):
        ...
        while True:
            ...
            await self.log("trace", f"Got message from queue: {request}")
            await self.__handle_request_message(request)

    @final
    async def __start(self):
        await self.log("trace", "Starting queue and handler...")
    ...
    async def __handle_request_message(self, request):
        await self.log("trace", f"Handling request message: {request}")
        await self.handle_request_message(request)
        await self.log("trace", "Finished handling request message.")
    ...
    async def create_response_skeleton(self, request):
        ...
        if to_[0] != self.agent_name:
            await self.log(
                "warning", f"Message is not destined for this agent. Message: {request}"
            )
    ...
    async def handle_response_message(self, response):
        ...
        await self.log("trace", f"Responding: {response}")
    ...
    async def __finalize(self):
        await self.log("trace", "Finalizing agent...")
        await self.stop()
        await self.finalize()
        self.request_message_queue.shutdown(immediate=True)
        await self.log("trace", "Agent finalized.")
...
class AgentManager:
    ...
    async def log(cls, type_, message):
        await yaclaw.log.log("AgentManager", type_, message)

    @classmethod
    async def initialize(cls, agent_settings):
        await cls.log("trace", "Initializing agents...")
    ...
    async def start_all(cls):
        await cls.log("trace", "Starting all agents...")
        async with asyncio.TaskGroup() as tg:
        ...
        await cls.log("trace", "All agents started.")

    @classmethod
    async def finalize(cls):
        await cls.log("trace", "Finalizing all agents...")
        for agent_name, agent_instance in cls.__agent_dict.items():
            await agent_instance._Agent__finalize()
        await cls.log("trace", "All agents finalized.")
